...
class Augmenter(nn.Module):
    def __init__(
        self, noise_dim, latent_dim, input_dim=5000, n_dim=100, n_zim=1, p_drop=0.2
    ):
        super().__init__()
        ...
        self.batch_fc1 = nn.BatchNorm1d(
            num_features=self.fc1.out_features, eps=1e-10, momentum=moment, affine=False
        )
        self.fc2 = nn.Linear(self.fc1.out_features, self.fc1.out_features)
        self.batch_fc2 = nn.BatchNorm1d(
            num_features=self.fc2.out_features, eps=1e-10, momentum=moment, affine=False
        )
        self.fc3 = nn.Linear(self.fc1.out_features, n_dim)
        self.batch_fc3 = nn.BatchNorm1d(
            num_features=self.fc3.out_features, eps=1e-10, momentum=moment, affine=False
        )
        self.fc4 = nn.Linear(n_dim, n_dim)
        self.batch_fc4 = nn.BatchNorm1d(
            num_features=self.fc4.out_features, eps=1e-10, momentum=moment, affine=False
        )
        self.fc5 = nn.Linear(n_dim, n_dim // 5)
        self.batch_fc5 = nn.BatchNorm1d(
            num_features=self.fc5.out_features, eps=1e-10, momentum=moment, affine=False
        )
        self.fc5n = nn.Linear(n_dim + noise_dim, n_dim // 5)
        self.batch_fc5n = nn.BatchNorm1d(
            num_features=self.fc5n.out_features,
            eps=1e-10,
            momentum=moment,
            affine=False,
        )
        ...
        self.batch_fc_mu = nn.BatchNorm1d(
            num_features=self.fc_mu.out_features,
            eps=1e-10,
            momentum=moment,
            affine=False,
        )
        self.fc6 = nn.Linear(self.fc_mu.out_features, n_dim // 5)
        self.batch_fc6 = nn.BatchNorm1d(
            num_features=self.fc6.out_features, eps=1e-10, momentum=moment, affine=False
        )
        self.fc7 = nn.Linear(self.fc6.out_features, n_dim)
        self.batch_fc7 = nn.BatchNorm1d(
            num_features=self.fc7.out_features, eps=1e-10, momentum=moment, affine=False
        )
        self.fc8 = nn.Linear(n_dim, n_dim)
        self.batch_fc8 = nn.BatchNorm1d(
            num_features=self.fc8.out_features, eps=1e-10, momentum=moment, affine=False
        )
        self.fc9 = nn.Linear(n_dim, input_dim // 5)
        self.batch_fc9 = nn.BatchNorm1d(
            num_features=self.fc9.out_features, eps=1e-10, momentum=moment, affine=False
        )
        self.fc10 = nn.Linear(input_dim // 5, input_dim // 5)
        self.batch_fc10 = nn.BatchNorm1d(
            num_features=self.fc10.out_features,
            eps=1e-10,
            momentum=moment,
            affine=False,
        )
...
class Augmenter_smartseq(nn.Module):
    ...
    def forward(self, x, batched, scale=1.0):
        if batched:
            z = scale * torch.randn(
                x.shape[0],
                x.shape[1],
            ...
            x = F.relu(self.batch_fc1(self.fc1(self.dp(x)).permute(1, 2, 0))).permute(
                2, 0, 1
            )
            ...
            z = scale * torch.randn(
                x.shape[0], self.noise_dim, device=x.device, requires_grad=False
            )
            z = F.elu(self.bnz(self.noise(z)))
            x = F.relu(self.batch_fc1(self.fc1(self.dp(x))))
            x = F.relu(self.batch_fc2(self.fc2(x)))
