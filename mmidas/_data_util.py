from typing import Callable

import numpy as np
import matplotlib.pyplot as plt
import torch
from torch.utils.data import DataLoader, Dataset, random_split
from torchvision import datasets, transforms
from toolz.curried import *


def viz(xs):
    fig, ax = plt.subplots(1, count(xs))
    for i, x in enumerate(xs):
        ax[i].imshow(x)
        ax[i].axis("off")
    plt.show()


_dataset_registry = {}

TRANSFORM_MNIST = transforms.Compose([transforms.ToTensor(),
                                      transforms.Normalize((0.1307,), (0.3081,)),
                                      transforms.Lambda(lambda x: torch.flatten(x))])

def register_dataset(name, data_fn):
    _dataset_registry[name] = data_fn


def make_loaders(dataset, *args, **kwargs):
    return _dataset_registry[dataset](*args, **kwargs)


def make_mnist(fn):
    train_data = fn('data', train=True, download=True, transform=TRANSFORM_MNIST)
    test_data = fn('data', train=False, download=True, transform=TRANSFORM_MNIST)

    n = int(0.8 * len(train_data))
    train_data, val_data = random_split(train_data, [n, count(train_data) - n])

    return train_data, val_data, test_data


def _generic_load(fn, transform, batch_size, test_batch_size=None):
    if test_batch_size is None:
        test_batch_size = batch_size

    train_data = fn('data', train=True, download=True, transform=transform)
    test_data = fn('data', train=False, download=True, transform=transform)

    train_data, val_data = _split_data(train_data, 0.8)

    train_loader = DataLoader(train_data, batch_size=batch_size, shuffle=True)
    val_loader = DataLoader(val_data, batch_size=batch_size, shuffle=False)
    test_loader = DataLoader(test_data, batch_size=test_batch_size, shuffle=False)
    return train_loader, val_loader, test_loader

def _split_data(data, p):
    n = int(p * len(data))
    return random_split(data, [n, count(data) - n])


def _load_mnist(batch_size,  test_batch_size=None):
    return _generic_load(datasets.MNIST, TRANSFORM_MNIST, batch_size, test_batch_size)
    

def _load_cifar10(batch_size, test_batch_size=None):
    transform = transforms.Compose([transforms.ToTensor(),
                                    transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
    return _generic_load(datasets.CIFAR10, transform, batch_size, test_batch_size)


register_dataset('mnist', _load_mnist)
register_dataset('cifar10', _load_cifar10)
